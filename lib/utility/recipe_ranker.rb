  ...
  class RecipeRanker
    ...
    def create_movie_recipes_associations
      ...
      recipes.each do |recipe|
        ...
        mentions = shared_ingredients.map { |ing| ing.name}
        
        # Find out if the recipe shares more than two ingredients with movie_ingredients
        if (shared_ingredients.length > 1)
          movie_recipe = MoviesRecipesAssociation.new({movie: @movie, recipe: recipe, mentions: mentions, mentions_percentage: ((shared_ingredients.length.to_f / movie_ingredients.length.to_f ).to_f * 100)})
